...
for index, column in countriesDf.iterrows():
  countryDe = GoogleTranslator(source='en', target='fr').translate(text=str(column['Country']).strip())
...
for index, column in capitalsDf.iterrows():
  capitalDe = GoogleTranslator(source='en', target='fr').translate(text=str(column['Capital']).strip())
